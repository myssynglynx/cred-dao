...
exports.radialBackgroundFadeIn = radialBackgroundFadeIn;
exports.unBlur = unBlur;
exports.zipDown = zipDown;
exports.zipUp = zipUp;
...
function radialBackgroundFadeIn(element, style, r1, b1, g1, r2, b2, g2, a1, a2, duration) {
  const prefix1 = 'radial-gradiant(' + style + ', rgb(' + r1 + ',' + g1 + ',' + b1 + ',',
        prefix2 = '), rgb(' + r2 + ',' + g2 + ',' + b2 + ',';
  const elements = document.querySelectorAll(element);
  let start = performance.now();
  requestAnimationFrame(function animation(time) {
    let fractionOfTime = (time - start) / duration;
    if (fractionOfTime > 1) fractionOfTime = 1;
    let progress = easeInOut(fractionOfTime, 0, 1);
    elements.forEach(e => e.style.backgroundColor = prefix1 + progress * a1 + prefix2 + progress * a2 + '))');
    if (fractionOfTime < 1) requestAnimationFrame(animation);
  });
}
...
function zipDown(element, duration) {
  const elements = document.querySelectorAll(element);
  let start = performance.now();
  requestAnimationFrame(function animation(time) {
    let fractionOfTime = (time - start) / duration;
